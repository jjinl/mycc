...
nvm_stack_t nvm_float2stack(nvm_float_t val)
{
  nvm_union_t v;
  v.f[0]=val;
  nvm_stack_t res = v.i[0];
  if(res & NVM_IMMEDIATE_MASK)
    res |= NVM_IMMEDIATE_MASK>>1;
  else
    res &= ~(NVM_IMMEDIATE_MASK>>1);
  res &= ~NVM_IMMEDIATE_MASK;
  return res;
}
...
{
  if(val&(NVM_IMMEDIATE_MASK>>1))
    val |= NVM_IMMEDIATE_MASK;
  if(val&(NVM_IMMEDIATE_MASK>>2))
    val &= ~(NVM_IMMEDIATE_MASK>>1);
  else  
    val |= NVM_IMMEDIATE_MASK>>1;
  nvm_union_t v;
  v.i[0]=val;
...
void   vm_run(u16_t mref) {
  ...
  do {
    ...
    // duplicate top two stack items  (a,b -> a,b,a,b)
      ...
      u08_t skip = arg0.z.bh;

      DEBUGF("skip: %d\n", skip);
      tmp1 = ((nvmfile_read08(pc+skip+7)<<8) | 
	      nvmfile_read08(pc+skip+8));        // get low value
      tmp2 = ((nvmfile_read08(pc+skip+11)<<8) | 
	      nvmfile_read08(pc+skip+12));       // get high value
	...
	tmp2 = skip + 3 + 12 + ((arg0.tmp - tmp1)<<2);	
    ...
    // push item from constant pool
      ...
      pc_inc = 2;
      DEBUGF("ldc #"DBG16"\n", arg0.z.bh);
      stack_push(NVM_TYPE_CONST | arg0.z.bh);
    }
